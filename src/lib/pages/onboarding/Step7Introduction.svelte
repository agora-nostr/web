<script lang="ts">
  ...
  let { profileData, inviterPubkey, inviteRelay, onNext, onSkip }: Props =
    $props();
  ...
  const inviterName = $derived(
    inviterProfile?.displayName || inviterProfile?.name || "your inviter",
  );
  ...
  async function saveIntroduction() {
    ...
    try {
      ...
      const introData = {
        ...
        hashtags,
        mentionInviter:
          mentionInviter && inviterPubkey ? inviterPubkey : undefined,
      };
...
<div class="min-h-screen flex flex-col">
  <div
    class="flex-1 px-4 lg:px-8 py-6 max-w-[1400px] mx-auto w-full pb-32 lg:pb-6"
  >
    <div class="text-center mb-6">
      <h1 class="text-2xl lg:text-3xl font-bold mb-2">
        {$t("onboarding.step5Introduction.title")}
      </h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:h-[calc(100vh-200px)]">
      ...
      <div class="flex flex-col order-1 lg:order-2">
        <div class="lg:p-6">
          <label class="block font-semibold mb-3"
            >{$t("onboarding.step5Introduction.writeLabel")}</label
          >
          ...
          <div class="flex items-center justify-between mt-3">
            <div class="text-xs text-muted-foreground">
              {#if profileData.location}
                {$t("onboarding.step5Introduction.tipLocation", {
                  values: { location: profileData.location },
                })}
              {/if}
            </div>
            <div class="text-xs">
              ...
              >
                {charCount}
                {$t("onboarding.step5Introduction.characters")}
              </span>
            </div>
          </div>
          ...
          {#if inviterPubkey}
            <div
              class="mt-4 p-3 bg-primary-50 dark:bg-primary-900/10 border border-primary-200 dark:border-primary-800 rounded-lg"
            >
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2 text-sm">
                  {#if mentionInviter}
                    ...
                    <span class="text-muted-foreground">
                      {$t("onboarding.step5Introduction.inviterNotify", {
                  ...
                  {:else}
                    ...
                    <span class="text-muted-foreground">
                      {$t("onboarding.step5Introduction.inviterNoNotify", {
                        values: { name: inviterName },
                      })}
                    </span>
                  {/if}
                ...
                >
                  {mentionInviter
                    ? $t("onboarding.step5Introduction.inviterRemove")
                    : $t("onboarding.step5Introduction.inviterAddBack")}
                </button>
              </div>
            </div>
          {/if}
          ...
          <div class="hidden lg:flex gap-3 mt-6">
            ...
            >
              {publishing
                ? $t("onboarding.step5Introduction.publishing")
                : $t("onboarding.step5Introduction.postIntroduction")}
            </button>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-col order-2 lg:order-1">
        <h3
          class="font-semibold text-sm text-muted-foreground uppercase tracking-wide mb-4"
        >
          {$t("onboarding.step5Introduction.recentIntroductions")}
        </h3>
        <div
          class="space-y-3 overflow-y-auto flex-1 pr-2 max-h-[400px] lg:max-h-none"
        >
          {#each introductionPosts.events.slice(0, 10) as event (event.id)}
            <NoteCard {event} />
          {/each}
        </div>
      </div>
  ...
  <div
    class="lg:hidden fixed bottom-0 left-0 right-0 bg-card border-t border p-4 z-50"
  >
